    ...
    MySQL.query("SELECT * FROM bank_users WHERE charidentifier = @charidentifier AND name = @bankName",
        { ["@charidentifier"] = charidentifier, ["@bankName"] = bankName }, function(result)
            if result[1] then
                ...
                local allBanksResult = MySQL.query.await(
                    "SELECT * FROM bank_users WHERE charidentifier = @charidentifier",
                ...
                MySQL.query("SELECT * FROM bank_users WHERE charidentifier = @charidentifier AND name = @bankName",
                    { ["@charidentifier"] = charidentifier, ["@bankName"] = bankName }, function(result1)
                        if result1[1] then
...
AddEventHandler('vorp_bank:UpgradeSafeBox', function(costlot, maxslots, slotsBought, name, currentspace)
    ...
    VORPcore.NotifyRightTip(_source, T.success .. (costlot * slotsBought) .. " | " .. FinalSlots .. " / " .. maxslots,
        4000)
end)
...
AddEventHandler('vorp_bank:transfer', function(amount, fromBank, toBank)
    ...
    local queryResult = MySQL.query.await("SELECT * FROM bank_users WHERE charidentifier = @characterId;",
    ...
    if queryResult then
        ...
        if bankAccounts[fromBank].money > amount then
            ...
            local updateFromResult = MySQL.query.await(
                "UPDATE bank_users SET money = @newBalance WHERE charidentifier = @characterId AND name = @fromBank;",
                { ["@newBalance"] = newBalanceFrom, ["@characterId"] = characterId, ["@fromBank"] = fromBank })

            if updateFromResult then
                local updateToResult = MySQL.query.await(
                    "UPDATE bank_users SET money = @newBalance WHERE charidentifier = @characterId AND name = @toBank;",
                    { ["@newBalance"] = newBalanceTo, ["@characterId"] = characterId, ["@toBank"] = toBank })
                ...
                if updateToResult then
                    ...
                    local msg = string.format(T.transfer .. "%s $" .. T.to .. "%s" .. T.transferred, transferredAmount,
                        toBank)
        ...
        MySQL.query("SELECT * FROM bank_users WHERE charidentifier = @characterId AND name = @bankName",
            { ["@characterId"] = characterId, ["@bankName"] = bankName }, function(result)
                if result[1] then
                    ...
                    MySQL.update(
                        "UPDATE bank_users SET money=@newBalance WHERE charidentifier=@characterId AND name = @bankName",
                        { ['@characterId'] = characterId, ['@newBalance'] = newBalance, ['@bankName'] = bankName })
...
AddEventHandler('vorp_bank:depositgold', function(amount, bankName)
    ...
    if playerGold >= amount then
        ...
        MySQL.update(
            "UPDATE bank_users SET gold = gold + @amount WHERE charidentifier = @characterId AND name = @bankName",
            ...
            if result[1] then
                ...
                if bankBalance >= amount then
                    if not lastMoney[_source] or lastMoney[_source] ~= bankBalance then
                        ...
                        print("^1Potential cheating detected: player " ..
                            playerFullName .. " attempted repeated transactions with unchanged balance.^7")
                    end
                else
...
AddEventHandler('vorp_bank:withgold', function(amount, bankName)
    ...
    MySQL.query("SELECT gold FROM bank_users WHERE charidentifier = @characterId AND name = @bankName",
        { ["@characterId"] = characterId, ["@bankName"] = bankName }, function(result)
            if result[1] then
                ...
                if bankGold >= amount then
                    ...
                    MySQL.update(
                        "UPDATE bank_users SET gold = @newGoldBalance WHERE charidentifier = @characterId AND name = @bankName",
                        { ['@characterId'] = characterId, ['@newGoldBalance'] = newGoldBalance, ['@bankName'] = bankName })
                            ...
                            if result[1].items then
                                ...
                                if foundItem then
                                    ...
                                    MySQL.update(
                                        ...
                                        { ["@inv"] = json.encode(inv), ["@charidentifier"] = charidentifier, ["@name"] =
                                        name })
                                end
                            end
                            notpass = false
